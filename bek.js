/* ============================================================
   OLeX | BozorUZ — bek.js
   Telegram Mini App · OLX klon (Oshpaz, Do'kon, Dastafchik)
   Versiya 2.0
   ============================================================ */
...
const CAT_LABELS = {
  ...
  home:        '🏠 Uy jihozlari',
  cars:        '🚗 Avto',
  food_products: '🍕 Taomlar',
  shop_products: '🏪 Do\'kon',
  other:       '📦 Boshqa'
};
    ...
    if (!Array.isArray(r1) || r1.length === 0) {
      await apiPost(`${CFG.SHEETDB}?sheet=${CFG.SHEET_PRODUCTS}`, {
        data: [{
          ...
          created_at: new Date().toISOString(), food_category: '', shop_category: '', role: ''
        }]
      });
    ...
    if (!Array.isArray(r2) || r2.length === 0) {
      await apiPost(`${CFG.SHEETDB}?sheet=${CFG.SHEET_PROFILES}`, {
        data: [{
          ...
          telefon: '', shahar: '', emoji: '😊', role: ROLES.NORMAL,
...
async function setUserRole(role) {
  ...
  const roleNames = {
    chef: { name: '👨‍🍳 Oshpaz', icon: '👨‍🍳' },
    shopkeeper: { name: '🏪 Do\'konchi', icon: '🏪' },
    delivery: { name: '🚚 Dastafchik', icon: '🚚' }
  };
  
  const confirmMsg = `"${roleNames[role].name}" bo'lishni tasdiqlaysizmi?`;
  ...
  try {
    ...
    STATE.userRole = role;
    updateRoleUI();
    showMsg('roleStatus', `✅ Siz ${roleNames[role].name} sifatida ro'yxatdan o'tdingiz!`, 'success');
    
    setTimeout(() => {
      document.getElementById('roleStatus').classList.add('hidden');
    }, 3000);
    
  } catch(e) {
    showMsg('roleStatus', '❌ Xatolik: ' + e.message, 'error');
  }
}

function updateRoleUI() {
  const role = STATE.profile?.role || ROLES.NORMAL;
  STATE.userRole = role;
  
  const roleCard = document.getElementById('currentRoleCard');
  const roleIcon = document.getElementById('roleIcon');
  const roleText = document.getElementById('currentRoleText');
  
  const roleData = {
    [ROLES.NORMAL]: { icon: '👤', text: 'Oddiy foydalanuvchi' },
    [ROLES.CHEF]: { icon: '👨‍🍳', text: 'Oshpaz' },
    [ROLES.SHOPKEEPER]: { icon: '🏪', text: 'Do\'konchi' },
    [ROLES.DELIVERY]: { icon: '🚚', text: 'Dastafchik' }
  };
  
  if (role !== ROLES.NORMAL && roleCard) {
    roleCard.classList.remove('hidden');
    if (roleIcon) roleIcon.textContent = roleData[role].icon;
    if (roleText) roleText.textContent = roleData[role].text;
  } else if (roleCard) {
    roleCard.classList.add('hidden');
  }
  
  const myFoodsSection = document.getElementById('myFoodsSection');
  const myShopSection = document.getElementById('myShopSection');
  
  if (myFoodsSection) {
    myFoodsSection.classList.toggle('hidden', role !== ROLES.CHEF);
  }
  if (myShopSection) {
    myShopSection.classList.toggle('hidden', role !== ROLES.SHOPKEEPER);
  }
...
async function loadProfile() {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      STATE.userRole = data[0].role || ROLES.NORMAL;
    } else {
      STATE.profile = {
        user_id: uid,
      ...
      STATE.userRole = ROLES.NORMAL;
    }
  } catch(e) {
    console.error('loadProfile:', e);
    STATE.profile = { user_id: uid, username: STATE.tgUser.username || '', ism: '', familya: '', telefon: '', shahar: '', emoji: '😊', role: ROLES.NORMAL };
    STATE.userRole = ROLES.NORMAL;
  }
...
async function saveProfile() {
  ...
  const payload = {
    ...
    role:       STATE.profile?.role || ROLES.NORMAL,
    updated_at: new Date().toISOString()
  };
...
function loadFoodProducts() {
  STATE.foods = STATE.products.filter(p => p.category === 'food_products');
  renderFoodProducts();
}

function loadShopProducts() {
  STATE.shopProducts = STATE.products.filter(p => p.category === 'shop_products');
  renderShopProducts();
}
...
function renderFoodProducts() {
  ...
  if (STATE.currentFoodCat !== 'all') {
    filtered = filtered.filter(p => p.food_category === STATE.currentFoodCat);
  }
  
  if (!filtered || filtered.length === 0) {
    grid.innerHTML = '<div class="empty-state-full"><span class="big-emoji">🍕</span><p>Hali taomlar yo\'q</p></div>';
    return;
  }
...
function renderShopProducts() {
  ...
  let filtered = STATE.shopProducts;
  
  if (STATE.currentShopCat !== 'all') {
    filtered = filtered.filter(p => p.shop_category === STATE.currentShopCat);
  }
  
  if (!filtered || filtered.length === 0) {
    grid.innerHTML = '<div class="empty-state-full"><span class="big-emoji">🏪</span><p>Hali do\'kon mahsulotlari yo\'q</p></div>';
    return;
  }
  grid.innerHTML = filtered.map(p => productCardHTML(p)).join('');
}
...
function filterFoodCategory(cat) {
  STATE.currentFoodCat = cat;
  document.querySelectorAll('.food-categories .filter-tab').forEach(tab => {
    tab.classList.remove('active');
    if (tab.dataset.foodCat === cat) {
...
function filterShopCategory(cat) {
  ...
  document.querySelectorAll('.shop-categories .filter-tab').forEach(tab => {
    tab.classList.remove('active');
    if (tab.dataset.shopCat === cat) {
      tab.classList.add('active');
    }
  });
  renderShopProducts();
...
function updateFilterBadges() {
  ...
  if (STATE.currentCat !== 'all') {
    badges.push({ type: 'category', text: `📁 ${CAT_LABELS[STATE.currentCat] || STATE.currentCat}` });
  }
...
function updateFilterFabVisibility() {
  ...
  if (fab) {
    fab.style.display = (currentPage === 'page-home' || currentPage === 'page-food' || currentPage === 'page-shop') ? 'flex' : 'none';
  }
}
...
async function openDetail(id) {
  ...
  const contactBtns = isOwner
    ...
    : `
      ...
      <a href="https://t.me/${esc(sellerProfile?.username || '')}" target="_blank" class="btn-tg">💬 Telegram orqali yozish</a>
    `;
...
function renderMyFoods() {
  const uid = String(STATE.tgUser?.id || '');
  const mine = STATE.foods.filter(p => p.user_id === uid);
  const grid = document.getElementById('myFoodGrid');

  if (!grid) return;
  
  if (!mine.length) {
    grid.innerHTML = '<div class="empty-my">Hali taom yo\'q</div>';
    return;
  }
  ...
  grid.innerHTML = mine.map(p => {
    ...
    const imgHTML  = p.img_url
      ? `<img class="card-img" src="${esc(p.img_url)}" alt="" loading="lazy">`
      : `<div class="card-img-placeholder" style="aspect-ratio:4/3">🍕</div>`;
    return `
      <div class="my-prod-card" onclick="openDetail('${esc(p.id)}')">
        ${imgHTML}
        <span class="my-prod-days">${daysLeft > 0 ? `${daysLeft}k` : '❌'}</span>
...
function renderMyShopProducts() {
  const uid = String(STATE.tgUser?.id || '');
  const mine = STATE.shopProducts.filter(p => p.user_id === uid);
  const grid = document.getElementById('myShopGrid');

  if (!grid) return;
  
  if (!mine.length) {
    grid.innerHTML = '<div class="empty-my">Hali do\'kon mahsuloti yo\'q</div>';
    return;
  }
...
function getCatEmoji(cat) {
  const map = { electronics:'📱', livestock:'🐄', clothes:'👗', home:'🏠', cars:'🚗', food_products:'🍕', shop_products:'🏪', other:'📦' };
  return map[cat] || '📦';
}
